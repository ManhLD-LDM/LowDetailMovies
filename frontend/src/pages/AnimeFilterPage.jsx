...
function AnimeFilterPage() {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      ...
      try {
        const categories = await kkphimApi.getAllCategories();
  ...
  const handleSearchWithCategories = async (categories) => {
    ...
    setHasSearched(true);
    try {
      const response = await kkphimApi.filterAnimeByCategories(categories, 1, 500);
  ...
  const handleSearch = async (categories = selectedCategories) => {
    ...
    try {
      ...
      navigate(`/anime/filter?categories=${slugs}`, { replace: true });
    } catch (error) {
      console.error('Error filtering anime:', error);
    } finally {
      setLoading(false);
  ...
  const clearAll = () => {
    ...
    navigate('/anime/filter', { replace: true });
  };
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-gray-400">Chọn các thể loại để tìm anime phù hợp</p>
          </div>
          <button
        ...
        <div>
          <div className="mb-6">
            ...
            <p className="text-gray-400">
              Tìm thấy {filteredAnimes.length} anime phù hợp
            </p>
          </div>

          {loading ? (
            <MovieGrid movies={[]} loading={true} contentType="anime" />
          ) : filteredAnimes.length > 0 ? (
            <MovieGrid movies={filteredAnimes} loading={false} contentType="anime" />
          ) : (
            <div className="text-center py-16">
              <div className="text-6xl mb-4">😢</div>
              <p className="text-gray-400 text-lg">
                Không tìm thấy anime nào với các thể loại đã chọn
              </p>
              <button
                onClick={clearAll}
...
export default AnimeFilterPage;
